...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Codon failed to launch";
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("Codon System Specs (from CLI):\n{}", system_specs);
        return;
    }
...
#[derive(Parser, Debug)]
#[command(name = "codon", disable_version_flag = true, max_term_width = 100)]
struct Args {
...
fn watch_languages(fs: Arc<dyn fs::Fs>, languages: Arc<LanguageRegistry>, cx: &mut App) {
    ...
    cx.background_spawn(async move {
        let languages_src = Path::new("vendor/zed/crates/grammars/src");
